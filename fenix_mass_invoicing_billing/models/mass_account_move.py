...
class MassAccountMove(models.Model):
    ...
    invoice_date = fields.Date(string="Invoice / Bill date to :", default=fields.Date.today(), required=True, copy=False, 
        states={'scheduled': [('readonly', True)], 'processed': [('readonly', True)], 'cancel': [('readonly', True)]})
    template_id = fields.Many2one(comodel_name='mail.template', string='Use template', index=True,
        domain="[('model', '=', 'account.move')]", states={'scheduled': [('readonly', True)], 'processed': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    def _action_sent_move(self, move):
        ...
        account_invoice_send_id.composer_id.onchange_template_id_wrapper()
    ...
    def _action_sale_order_move_create(self, orders, **kw):
        ...
        new_moves = orders._create_invoices(
        ...
        if orders:
            ...
            if (confirm_forcefully or (confirm_by_policy and (not is_proforma))):
                ...
                if kw.get('no_create_self_confirm_only', False):
                    ...
                    account_moves |= new_created_moves
    ...
    def _process(self, partner_id, orders, **kw):
        ...
        if not orders:
            return False
        if self.model_id.model == 'purchase.order' and self.env[self.model_id.model].search([('id', 'in', orders.ids)]):
            if self.partner_with_threshold and orders._check_is_partner_wo_invoicing_threshold(partner_id):
                return False
        elif self.model_id.model == 'sale.order':
            if self.partner_with_threshold and orders._check_is_partner_wo_invoicing_threshold(partner_id):
                return False
        else:
            return False
