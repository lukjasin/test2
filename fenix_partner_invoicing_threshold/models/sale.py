...
class SaleOrder(models.Model):
    ...
    def _check_is_partner_wo_invoicing_threshold(self, group_partner):
        return (float_compare(
            sum(self.filtered(lambda so: so.partner_id.id == group_partner.id).mapped('amount_total')),
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if 'threshold_partner_id' in groupby:
            ...
            for partner_id, sale_order_ids in groupby_orders():
                if partner_id.invoicing_threshold_amount and\
                    sale_order_ids._check_is_partner_wo_invoicing_threshold(partner_id):
                    order_without_threshold |= sale_order_ids
        ...
        if 'threshold_wo_email_partner_id' in groupby:
            ...
            for partner_id, sale_order_ids in groupby_orders():
                if partner_id.invoicing_threshold_amount and\
                not sale_order_ids._check_is_partner_wo_invoicing_threshold(partner_id):
                    threshold_without_email |= sale_order_ids.filtered(lambda x: not x.partner_id.email and x.invoice_status == 'to invoice')
            domain = expression.AND([domain, [('id', 'in', threshold_without_email.ids)]])

        return super(SaleOrder, self).read_group(domain=domain, fields=fields, groupby=groupby,
