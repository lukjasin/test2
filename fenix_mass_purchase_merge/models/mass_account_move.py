...
class MassAccountMove(models.Model):
    ...
    proforma_date = fields.Date(string="Proforma Date :", default=fields.Date.today(), required=True, copy=False, 
        states={'scheduled': [('readonly', True)], 'processed': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    def _action_rfq_send(self, order):
        ...
        if is_sent_mail:
            ...
            if self.move_confirmation in ['create_sent', 'create_confirm_sent'] and order.state not in ['cancel']:
                order.write({
                    'proforma_status': self.env.context.get('proforma_status') if self.env.context.get('proforma_status') else '',
                    'proforma_date': self.env.context.get('proforma_date') if self.env.context.get('proforma_date') else '',
                })
            return True
        return False
    ...
    def _action_purchase_order_move_create(self, orders, **kw):
        ...
        if orders:
            ...
            proforma_status = ''
            if self.move_confirmation in ('create', 'create_confirm'):
                proforma_status = 'create'
            else:
                proforma_status = 'create_send'
            if confirm_by_policy and is_proforma and self.is_generate_proforma and self.is_grouped_move:
                ...
                if not new_order:
                    ...
                    new_order = po_merge_id.with_context(active_ids=orders.ids, proforma_status=proforma_status, proforma_date=self.proforma_date).merge_purchase_orders()
                if is_confirm_necessary and new_order:
                    new_order.button_confirm()
                    new_order.proforma_date = self.proforma_date
                    new_order.proforma_status = proforma_status
                self.with_context(proforma_status=proforma_status, proforma_date=self.proforma_date)._action_rfq_send(new_order)
        return moves
